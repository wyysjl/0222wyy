...
void DrawWidget::drawShape(const QPointF ptStart,const QPointF ptEnd,const ST::ShapeType drawType)
{
    ...
    case ST::Triangle:{
        ...
        break;
    case ST::Diamond:{
        QPointF poin1((ptStart.x()+ptEnd.x())/2,ptStart.y());
        QPointF poin2(ptEnd.x(),(ptEnd.y()+ptStart.y())/2);
        QPointF poin3((ptStart.x()+ptEnd.x())/2,ptEnd.y());
        QPointF poin4(ptStart.x(),(ptStart.y()+ptEnd.y())/2);

        QVector<QPointF> poins;
        poins<<poin1<<poin2<<poin3<<poin4;
        painter.drawPolygon(poins);
    }
        break;
    case ST::Text:{
